...
=======
		...
		#self.validate_sales_invoice_no()
...
=======
	...
	def validate_vat_duties_charges(self):
		...
		#msgprint(_("{0}-Outstanding").format(self.outstanding_amount))
		
		if self.vat != None:
			self.outstanding_amount += self.vat
			
		if self.duties != None:
			self.outstanding_amount += self.duties
			
		if self.other_charges !=None:
			self.outstanding_amount += self.other_charges
	
		if self.advance_received != None:
			self.total_advance = self.advance_received
			self.outstanding_amount = self.outstanding_amount - self.advance_received
			
		if self.discount_amount != None:
			self.outstanding_amount = self.outstanding_amount - self.discount_amount
			#msgprint(_("{0}-Outstanding minus discount").format(self.outstanding_amount))
			
		self.grand_total = self.outstanding_amount
		self.base_grand_total = self.outstanding_amount * self.conversion_rate
		
		self.net_total = self.outstanding_amount
		self.base_net_total = self.outstanding_amount * self.conversion_rate
		
		self.rounded_total = self.outstanding_amount
		self.base_rounded_total = self.outstanding_amount * self.conversion_rate
		
		self.in_words = money_in_words(self.grand_total, self.currency)
		self.base_in_words = money_in_words(self.base_grand_total, 'AED')
		
		self.outstanding_amount = self.outstanding_amount * self.conversion_rate
		
	def after_percentage(self):
		if self.percentage != None:
			gt = flt(self.total)*flt(self.percentage)/100
			gt_new = flt(self.total) - flt(gt)
			self.grand_total = gt_new
			self.base_grand_total = gt_new * self.conversion_rate
		
			self.net_total = gt_new
			self.base_net_total = gt_new * self.conversion_rate
		
			self.rounded_total = gt_new
			self.base_rounded_total = gt_new * self.conversion_rate
		
			self.in_words = money_in_words(self.grand_total, self.currency)
			self.base_in_words = money_in_words(self.base_grand_total, 'AED')
		
			self.outstanding_amount = gt_new  * self.conversion_rate
			msgprint("Grand Total" +str(self.grand_total))				
...
=======
	...
	##new addition
	def validate_sales_invoice_no(self):
		si_no = self.naming_series
		sino = frappe.db.sql("select name from `tabSales Invoice` where naming_series = %s",(self.naming_series))
		if len(sino) > 0:
			for data in sino:
				#frappe.throw(_("Sales Invoice: {0}").format(data[0]))
				if data[0] != self.name:
					frappe.throw(_("Sales Invoice with naming_series {0} already exists").format(self.naming_series))
...
=======
		income_acc = ""
		base_net_amount = 0.00
		net_amount = 0.00
		account_currency = ""
		cost_center = ""
		for item in self.get("items"):
			income_acc = item.income_account
...
=======
		...
		base_net_amount += self.vat * self.conversion_rate
		net_amount += self.vat
		base_net_amount += self.duties * self.conversion_rate
		net_amount += self.duties
		base_net_amount += self.other_charges * self.conversion_rate
		net_amount += self.other_charges
		base_net_amount = base_net_amount - (self.advance_received * self.conversion_rate)
		net_amount = net_amount - self.advance_received
		
		gl_entries.append(
			self.get_gl_dict({
				"account": income_acc,
				"against": self.customer,
				"credit": base_net_amount,
				"credit_in_account_currency": base_net_amount \
					if account_currency==self.company_currency else net_amount,
